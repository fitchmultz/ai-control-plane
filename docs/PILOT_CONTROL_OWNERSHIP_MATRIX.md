...
| SIEM ingestion | Normalized evidence schema, detections, and query validation | Onboard the feed into the customer SIEM, retention, alert routing, and SOC workflow | Tune mappings, detections, and investigation runbooks | `make validate-detections`; `make validate-siem-schema`; customer SIEM screenshots or test cases |
