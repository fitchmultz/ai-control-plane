...
| Check certificate lifecycle | `make cert-status DOMAIN=gateway.example.com` |
| Trigger certificate renewal | `make cert-renew DOMAIN=gateway.example.com THRESHOLD_DAYS=30` |
...
Use the typed certificate lifecycle surface instead of raw Docker-volume manipulation:
...
For host-first TLS deployments, install the automated renewal timer:
...
Controlled renewals preserve rollback artifacts under `demo/logs/cert-renewals/`.

See [docs/deployment/CERTIFICATE_LIFECYCLE.md](deployment/CERTIFICATE_LIFECYCLE.md) and [docs/deployment/TLS_SETUP.md](deployment/TLS_SETUP.md) for the full TLS and certificate contract.
