...
| --- | --- | --- | --- |
| 15 | **Validate off-host backup and replacement-host recovery.** Remove the biggest gap in the current single-node production story before broadening claims. | The current supported path still allows host-loss or storage-loss events to take out both the runtime and local backup artifacts. Off-host recovery proof is more valuable right now than cloud expansion. | The repo documents and validates an off-host backup-copy contract, replacement-host recovery workflow, and readiness/handoff evidence showing the deployment can be rebuilt from off-host inputs rather than same-host artifacts only. |
