...
complete -c acpctl -n '__fish_use_subcommand' -a 'helm' -d 'Helm chart validation and smoke gates'
...
complete -c acpctl -n '__fish_seen_subcommand_from host' -a 'secrets-refresh' -d 'Validate and sync canonical host secrets into the Compose runtime env file'
...
complete -c acpctl -n '__fish_seen_subcommand_from helm' -a 'validate' -d 'Validate Helm deployment surfaces'
complete -c acpctl -n '__fish_seen_subcommand_from helm' -a 'smoke' -d 'Run truthful Helm smoke validation'
