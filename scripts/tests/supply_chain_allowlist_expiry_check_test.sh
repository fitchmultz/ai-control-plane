...
assert_case_stderr_contains() {
    ...
    if [[ "$exit_code" -ne 1 ]]; then
        test_fail "$name (expected exit 1, got $exit_code)"
        return
    fi
    ...
    if grep -Fq "$expected" "$stderr_file"; then
        ...
        test_fail "$name (missing stderr: $expected)"
        echo "--- stderr ---"
        cat "$stderr_file" || true
    fi
}
...
cat >"$TMP_ROOT/fail-with-image.json" <<EOF
{"allowlist":[{"id":"CVE-3","package":"pkg","image":"registry.example/app@sha256:abc123","expires_on":"$fail_date","ticket":"SEC-3"}]}
EOF
...
assert_case_stderr_contains "fail output includes image identity" "$TMP_ROOT/fail-with-image.json" "CVE-3:pkg @ registry.example/app@sha256:abc123"
